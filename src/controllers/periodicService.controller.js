const { periodicServiceModel } = require("../models");
...
const createPeriodicService = async (req, res, next) => {
  try {
    const dataInSequence = await generateImageName(req);

    await uploadImage(req, dataInSequence.imgShortId);
    const resData = await periodicServiceModel.create(
      dataInSequence.combineData
    );
...
module.exports = { createPeriodicService };
